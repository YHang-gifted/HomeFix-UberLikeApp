...
### SEC-0010 — Reset token in a URL (the password-reset magic link)

> Not a fix to a shipped vulnerability — the hardening that a **new** feature required, recorded so it is not quietly removed later as "not doing anything".

- **Date:** 2026-07-13
- **Category:** secrets-exposure
- **Severity:** medium (would be high if the mitigations were removed)
- **Affected area:** `server/src/middlewares/securityHeaders.ts` (applied in `server/src/app.ts` to all responses), `app-expo/App.tsx` (`stripResetCodeFromUrl`), `app/src/features/auth/resetLink.ts`
- **Vulnerability:** Slice 183 puts the plaintext password-reset token into a URL (`/?reset=<token>`) so the user clicks a link instead of copying 64 hex characters by hand. A secret in a URL has two well-known escape routes, and both were open here. **(1) `Referer`:** the page that link opens loads the **Google Maps JS SDK**; with no referrer policy the browser attaches the _full_ URL — token included — as the `Referer` header on that third-party request. The token we take such care to keep out of our own logs would land in someone else's. **(2) The address bar:** left in place, the token is written to browser history and session restore, is what gets copied when a person shares "the page I'm on", and remains in the tab long after the reset is finished.
- **Root cause:** Not a mistake so much as the cost of the feature: any magic link buys UX with a secret in a URL. The app had also never set a single security header — no `Referrer-Policy`, no `nosniff`, no framing policy — so there was nothing already in place to catch it.
- **Canonical fix:** **The secret must survive exactly one hop.** (a) Server sends `Referrer-Policy: no-referrer` on **every** response (there is no analytics or affiliate flow that needs a `Referer`, so nothing is traded away); `X-Content-Type-Options: nosniff` and `X-Frame-Options: DENY` are set in the same middleware. (b) The app strips the token from the URL the instant it reads it, via `history.replaceState` — in place, so Back cannot restore it — and only _after_ the token has reached the screen that can use it (stripping unconditionally would destroy the only copy for a signed-in user who opened the link). (c) The token keeps its full 64 hex characters of entropy: shortening it to a friendlier code is the tempting move and is **forbidden**, because `resetPassword` resolves a token by the token alone (`findByTokenHash`) with no email to scope it — a 6-digit code could be guessed into _whichever_ account had a reset pending, with odds that improve as the userbase grows. Rule for the class: **if a secret must travel in a URL, it gets `no-referrer`, it gets stripped from history on arrival, and it keeps full entropy.**
